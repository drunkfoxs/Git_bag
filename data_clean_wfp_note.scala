...
//设置行分割符，读入原始文件
val INPUT_FILE = "/home/beauty/docs/data_source/clean_raw_data.csv"
val LINE = "\\ |\n"
...
val CONF_FILE = "/home/beauty/docs/data_source/conf_data_clean.txt"
val SEP = "_\\|"
...
val data_loc_form = data_loc.map(x => x.map{case (y,`int_id_loc`) => (y.split("\\.", -1)(0), int_id_loc)
					    ...
					    case z => z})
//读入网元类型对应的权重数据
//-val OBJECT_CLASS_FILE = "/home/beauty/docs/data_source/AWJ_NETSOURCE.csv"
...
val stad_flag_filt = alm_id_filt.filter(x => x.filter(y => y._2 == stad_flag_loc)(0)._1 == "2")
    //将int_id、alarm_id拼接起来，再加上时间，可唯一表示一条告警
val int_alm_data_loc = stad_flag_filt.map(x => ((x.filter(y => y._2 == int_id_loc || y._2 == alm_id_loc).map(_._1).reduce(_ + "_" + _), int_id_loc), x.filter(z => z._2 != int_id_loc && z._2 != alm_id_loc))).map(t => t._1 +: t._2)
    //过滤掉不符合业务的网元类型，然后按照网元类型、告警级别来计算告警信息的权重,AHP
...
//分窗函数，输入一个字符型的时间，输出一个始末时间对的集合（一个时间对对应一个窗口）
...
val OUTPUT_FILE = "/home/beauty/docs/data_source/CLEANED_8_9"
