...
public class NapaPcapReader implements Iterable<Packet>  {
	...
	public NapaPcapReader(DataInputStream is) throws IOException {
		this.is = is;
	...
	// Only use this constructor for testcases
		protected NapaPcapReader(LinkType lt) {
			this.is = null;
			linkType = lt;
		}
		...
		private Packet nextPacket() {
			logger.info("Start from packet parsing:  This is nextPacket() ");
		...
		protected Packet createPacket() {
			logger.info("Start from packet parsing:  This is processPacketPayload(Packet packet, byte[] payload)");
			return new Packet();
		}
		...
		private byte[] buildTcpAndUdpPacket(Packet packet, byte[] packetData, int ipProtocolHeaderVersion, int ipStart, int ipHeaderLen, int totalLength) {
			...
			logger.info("Start from packet parsing:  This is buildTcpAndUdpPacket(Packet packet, byte[] packetData, int ipProtocolHeaderVersion, int ipStart, int ipHeaderLen, int totalLength)");
			...
			if (PROTOCOL_UDP.equals(protocol)) {
				...
				System.out.println("=== Start parsing TCP packet ===");
		...
		protected byte[] readPayload(byte[] packetData, int payloadDataStart, int payloadLength) {
			if (payloadLength < 0) {
				logger.info("Malformed packet - negative payload length. Returning empty payload.");
				return new byte[0];
			}
			if (payloadDataStart > packetData.length) {
				logger.info("Payload start (" + payloadDataStart + ") is larger than packet data (" + packetData.length + "). Returning empty payload.");
				return new byte[0];
			}
			if (payloadDataStart + payloadLength > packetData.length) {
				if (payloadDataStart + payloadLength <= snapLen) // Only corrupted if it was not because of a reduced snap length
					logger.info("Payload length field value (" + payloadLength + ") is larger than available packet data (" 
		...
		private class PacketIterator implements Iterator<Packet> {
			...
			public boolean hasNext() {
				logger.info("Start from packet parsing:  This is hasNext()");
				fetchNext();
				if (next != null)
					return true;
			...
			@Override
			public Packet next() {
				logger.info("Start from packet parsing:  This is next()");
				fetchNext();
				try {
					return next;
				} finally {
